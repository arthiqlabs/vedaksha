...
fn planet_name(index: usize, language: &str) -> PyResult<String> {
    ...
    Ok(vedaksha_engine::locale::planets::planet_name(index, lang).to_string())
}
...
fn sign_name(index: usize, language: &str) -> PyResult<String> {
    let lang = parse_language(language)?;
    Ok(vedaksha_engine::locale::signs::sign_name(index, lang).to_string())
}
...
fn nakshatra_name_i18n(index: usize, language: &str) -> PyResult<String> {
    let lang = parse_language(language)?;
    Ok(vedaksha_engine::locale::nakshatras::nakshatra_name(index, lang).to_string())
}
...
fn parse_language(s: &str) -> PyResult<vedaksha_engine::locale::Language> {
    use vedaksha_engine::locale::Language;
    match s.to_lowercase().as_str() {
